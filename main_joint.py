...
from util.send_mail import Email
...
def main(args):
    ...
    if args.email and utils.is_main_process():
        email = Email()
        title = f"Start to train on {args.dataset_file} with backbone {args.backbone} {train_name}"
        content = str(args)
        email.send_msg(title, content)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.email and utils.is_main_process():
            title = f"Epoch {epoch} on {args.dataset_file} backbone {args.backbone} {train_name}"
            content = str(log_stats)+content
            email.send_msg(title, content)
