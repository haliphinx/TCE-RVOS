...
class ModulatedDetection(torchvision.datasets.CocoDetection):
    def __init__(self, img_folder, ann_file, num_frames, transforms, return_masks, args):
        super(ModulatedDetection, self).__init__(img_folder, ann_file)
        self._transforms = transforms
        self.prepare = ConvertCocoPolysToMask(return_masks)
        self.num_frames = num_frames
        self.f_extra = args.f_extra
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:
            ...
            assert numinst == 1
            for t in range(self.num_frames - 1 + 2 * self.f_extra):
            ...
            output_inst_masks = np.stack(output_inst_masks, axis=0)
            # if self.f_extra != 0:
            #     output_inst_masks = output_inst_masks[self.f_extra:-self.f_extra]

            output_inst_masks = torch.from_numpy(output_inst_masks)         
        ...
        clip_target(target, self.f_extra)
