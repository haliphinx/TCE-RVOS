'''
Inference code for ReferFormer, on MeViS
...
def main(args):
    ...
    root = '/media/xhu/Study2/data/MeViS'
    ...
    assert len(video_list) == 140, 'error: incorrect number of validation videos'
    ...
    for video in video_list:
        ...
        for i in range(num_expressions):
            ...
            if args.keep_fps:
                ...
                all_pred_masks = []
                all_pred_masks_dict = {}
                # step = args.num_frames - 1
                step = 20
                for img_id in range(0, video_len, step):
                    imgs = []
                    img_len = 0
                    img_start_id = img_id
                    if img_start_id + step > video_len:
                        img_start_id = video_len - step
                    img_ids = [n for n in range(img_start_id, img_start_id + step)]
                    if args.f_extra != 0:
                        for t in range(args.f_extra):
                            first_idx = img_ids[0]
                            last_idx = img_ids[-1]
                            img_ids = [max(0,first_idx - 1)] + img_ids
                            img_ids = img_ids + [min(video_len - 1,last_idx + 1)]
            ...
            #         max_inds = max_ind.repeat(img_len - 2*args.f_extra)
            #         pred_masks = pred_masks[range(img_len - 2*args.f_extra), max_inds, ...] # [t, h, w]
            ...
            #         all_pred_logits = pred_logits[range(img_len - 2*args.f_extra), max_inds] 
            #         all_pred_boxes = pred_boxes[range(img_len - 2*args.f_extra), max_inds]   
            #         all_pred_ref_points = pred_ref_points[range(img_len - 2*args.f_extra), max_inds] 
            #         # all_pred_masks.append(pred_masks)
            #         for idx in range(step):
            #             if idx + img_start_id in all_pred_masks_dict:
            #                 all_pred_masks_dict[idx + img_start_id].append(pred_masks[idx])
            #             else:
            #                 all_pred_masks_dict[idx + img_start_id] = [pred_masks[idx]]

            #     for img_id in range(video_len):
            #         if len(all_pred_masks_dict[img_id]) > 0:
            #             temp_mask = all_pred_masks_dict[img_id][0] > args.threshold
            #             all_pred_masks.append(temp_mask)
            #         # elif len(all_pred_masks_dict[img_id]) == 0:
            #         #     print("no result for ", img_id)
            #         else:
            #             temp_mask = np.mean(np.stack(all_pred_masks_dict[img_id]), axis=0)
            #             all_pred_masks.append(temp_mask > args.threshold)

            #     all_pred_masks = np.stack(all_pred_masks,axis=0)
            #     assert all_pred_masks.shape[0] == video_len, "the mask number is not equal to the video length!"
            # else:
            #     # store images
            #     imgs = []
            ...
            #     # memory
            #     # memorys = outputs["memory"].detach().cpu().numpy()
            #     # print(memorys.shape)
            #     # assert False
            ...
            #     # save memory
            #     # mem = memorys[j]
            #     # memory_file = os.path.join(save_path, frame_name + ".npy")
            #     # np.save(memory_file,mem)
            # # except:
            # #     print(video_name, i)
            # #     continue
        with lock:
