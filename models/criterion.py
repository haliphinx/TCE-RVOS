...
class SetCriterion(nn.Module):
    ...
    def loss_vis(self, outputs, targets, indices, num_boxes, log=True):

        assert 'pred_visible' in outputs
        src_vis = outputs['pred_visible']
        _, nf, nq = src_vis.shape[:3]
        
        matched_idx = indices[0][0]
        src_vis_matched = torch.index_select(src_vis, 2, matched_idx).squeeze(-1)
        
        src_vis = rearrange(src_vis, 'b t q k -> b (t q) k')

        # judge the valid frames
        valid_indices = []
        valids = torch.cat([target['valid'].type(torch.int64) for target in targets]).unsqueeze(-1).unsqueeze(0)

        
        target_vis_onehot = torch.zeros([src_vis_matched.shape[0], src_vis_matched.shape[1], src_vis_matched.shape[2]+1],
                                            dtype=src_vis_matched.dtype, layout=src_vis_matched.layout, device=src_vis_matched.device)

        target_vis_onehot.scatter_(2, 1-valids, 1)
        
        target_vis_onehot = target_vis_onehot[:,:,:-1]

        loss_ce = sigmoid_focal_loss(src_vis_matched, target_vis_onehot, nf, alpha=self.focal_alpha, gamma=2) * src_vis.shape[1]
        losses = {'loss_vis': loss_ce}
    ...
    def forward(self, outputs, targets):
        ...
        for loss in self.losses:
            # if targets[0]["instance_check"] == 0 and loss in inv_list:
            #     continue
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    ...
                    #      continue
                    kwargs = {}
                    if loss == 'labels' or loss == 'visible':
