...
class CrossModalFPNDecoder(nn.Module):
    def __init__(self, feature_channels: List, conv_dim: int, mask_dim: int, dim_feedforward: int = 2048, norm=None, vlblock = False):
        """
        Args:
        ...
        if self.vlblock:
            ...
            print("Using the FPN without text transformer.")
    ...
    def forward_features_no_transf(self, features, poses, memory):
        # nf: num_frames 
        # print("Using the FPN without text transformer.")
        # [::-1] means flip over the list
        for idx, (mem, f, pos) in enumerate(zip(memory[::-1], features[1:][::-1], poses[1:][::-1])): # 32x -> 8x
            lateral_conv = self.lateral_convs[idx]
            output_conv = self.output_convs[idx]

            # NOTE: here the (h, w) is the size for current fpn layer
            cur_fpn = lateral_conv(mem)
    ...
    def forward(self, features, text_features, pos, memory, nf):
        ...
        if self.vlblock:
            ...
            y = self.forward_features_no_transf(features, pos, memory)
