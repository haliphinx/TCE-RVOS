...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, cost_class: float = 1, cost_bbox: float = 1, cost_giou: float = 1,
                       cost_mask: float = 1, cost_dice: float = 1, cost_vis: float = 1, num_classes: int = 1, masks = True, vis = False):
        """Creates the matcher
    ...
    def forward(self, outputs, targets):
        ...
        """
        src_logits = outputs["pred_logits"] # [B, T, q, num_class]
        src_boxes = outputs["pred_boxes"]   
        src_masks = outputs["pred_masks"]
        if self.vis:
            src_visibles = outputs["pred_visible"]  # [B, T, q, 1]
        ...
        for i in range(bs): 
            ...
            # instance_check = targets[i]["instance_check"]
            instance_check = 1

            # class cost
            # we average the cost on valid frames
            cost_class = []
            C = torch.zeros((nq,1), device = out_prob.device)
            for t in range(nf):
                if tgt_valid[t] == 0:
                    continue
                ...
                # print(out_prob_split,tgt_ids_split)
            ...
            # visible cost
            if self.vis:
                out_vis = src_visibles[i].sigmoid()
                cost_vis = []
                for t in range(nf):
                    out_vis_split = out_vis[t] # [q, 1]
                    # print(out_vis_split.shape)
                    tgt_valid_split = tgt_valid[t].unsqueeze(0).long()
                    # print(tgt_valid_split)
                    # assert False
                    # # for invalid images, predict 0
                    # if tgt_valid_split == 0:
                    #     out_vis_split = 1 - out_vis_split

                    # Compute the visibility cost.
                    alpha = 0.25
                    gamma = 2.0
                    neg_cost_vis = (1 - alpha) * (out_vis_split ** gamma) * (-(1 - out_vis_split + 1e-8).log())
                    pos_cost_vis = alpha * ((1 - out_vis_split) ** gamma) * (-(out_vis_split + 1e-8).log())
                    cost_vis_split = pos_cost_vis[:, [0]] - neg_cost_vis[:, [0]]
                    cost_vis.append(cost_vis_split)
                
                cost_vis = torch.stack(cost_vis, dim=0).mean(0)  # [q, 1]
                C += self.cost_vis * cost_vis
            ...
            if self.masks:
                if instance_check == 1:
                    ...
                    # print(out_mask.transpose(0, 1).shape, tgt_mask.unsqueeze(0).shape)
                    # assert False
