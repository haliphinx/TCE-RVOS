...
from .tce_deformable_transformer import build_deforamble_transformer
...
class ReferFormer(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, 
                    ...
                    freeze_text_encoder=False, rel_coord=True, args = None):
        ...
        if self.args.vis_loss:
            self.visible_embed.bias.data = torch.ones(1) * bias_value
        ...
        if with_box_refine:
            ...
            if self.args.vis_loss:
                self.visible_embed = _get_clones(self.visible_embed, num_pred)
            ...
            if self.args.vis_loss:
                self.visible_embed = nn.ModuleList([self.visible_embed for _ in range(num_pred)])
        ...
        # self.tokenizer = RobertaTokenizerFast.from_pretrained('./models/tokenizer/')
        # self.text_encoder = RobertaModel.from_pretrained('./models/text_encoder/')
        ...
        self.pixel_decoder = CrossModalFPNDecoder(feature_channels=feature_channels, conv_dim=hidden_dim, 
                                                  mask_dim=mask_dim, dim_feedforward=dim_feedforward, norm="GN", vlblock=args.vlblock)
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        features, pos = clip_nestedtensor(features, pos, 0)

        # pos = pos[self.args.f_extra:-self.args.f_extra] 
        ...
        text_embed = repeat(text_sentence_features, 'b c -> b t c', t=t)
        ...
        if self.args.contrastive:
            contrastive_val = self.contrastive_cal(b, enc_outputs_class, text_sentence_features)
        # print(enc_outputs_class.shape)
        # print(contrastive_val.shape, contrastive_val)
        # assert False
        ...
        for lvl in range(hs.shape[0]):
            ...
            if self.args.vis_loss:
                outputs_visible = self.visible_embed[lvl](hs[lvl])
            ...
            if self.args.vis_loss:
                outputs_visibles.append(outputs_visible)
        ...
        if self.args.vis_loss:
            outputs_visible = torch.stack(outputs_visibles)
            outputs_visible = rearrange(outputs_visible, 'l (b t) q k -> l b t q k', b=b, t=t)
        ...
        if self.args.vis_loss:
            out['pred_visible'] = outputs_visible[-1] # [batch_size, time, num_queries_per_frame, 2]
        if self.args.contrastive:
            out['contrastive'] = contrastive_val
        # Segmentation
        ...
        if self.aux_loss:
            if self.args.vis_loss:
                out['aux_outputs'] = self._set_aux_loss(outputs_class, outputs_coord, outputs_seg_masks, outputs_visible)
            else:
        ...
        out["memory"] = enc_outputs_class
        return out

    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_coord, outputs_seg_masks, outputs_visible = None):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        if outputs_visible is None:
            return [{"pred_logits": a, "pred_boxes": b, "pred_masks": c} 
                for a, b, c in zip(outputs_class[:-1], outputs_coord[:-1], outputs_seg_masks[:-1])]
        return [{"pred_logits": a, "pred_boxes": b, "pred_masks": c, "pred_visible": d} 
                for a, b, c, d in zip(outputs_class[:-1], outputs_coord[:-1], outputs_seg_masks[:-1], outputs_visible[:-1])]
    ...
    def contrastive_cal(self, bs, vis_memory, text_sentence_features):
        cos = nn.CosineSimilarity(dim=2, eps=1e-6)
        vis_memory = rearrange(vis_memory, '(b t) s c -> b t s c', b=bs)
        text_feat = rearrange(text_sentence_features, '(b t) c -> b t c', b=bs)
        vis_memory = torch.mean(vis_memory, 2)
        b, t, c = vis_memory.shape
        text_feat = text_feat.repeat(1,t,1)
        output = cos(vis_memory, text_feat)
        # print(output.shape, output)
        return output

def clip_nestedtensor(nts, poses, f_extra):
    '''
    clip out nestedtensor to fit the following networks.
    '''
    if f_extra != 0:
        assert len(nts) == len(poses), "Pose size is not same as tenser size."
        for idx in range(len(nts)):
            nts[idx].tensors = nts[idx].tensors[f_extra:-f_extra]
            nts[idx].mask = nts[idx].mask[f_extra:-f_extra]
            poses[idx] = poses[idx][f_extra:-f_extra]
...
def build(args):
    ...
    elif 'x3d' in args.backbone:
        from .x3d import build_x3d_backbone
        backbone = build_x3d_backbone(args)
    else:
    ...
    if args.vis_loss:
        weight_dict['loss_vis'] = args.vis_loss_coef
