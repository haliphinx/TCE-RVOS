...
def main(args):
    ...
    # Here to use less video for testing
...
def sub_processor(lock, pid, args, data, save_path_prefix, save_visualize_path_prefix, img_folder, video_list):
    ...
    for video in video_list:
        ...
        for anno_id in range(4): # 4 annotators
            ...
            anno_bboxes = []  # [num_obj+1, video_len, 4], +1 for background
            ...
            for obj_id in range(num_obj): 
                ...
                # 3. for each clip
                clip_size = 32
                for clip_id in range(0, video_len, clip_size):
                    frames_ids = [x for x in range(video_len)]
                    clip_frames_ids = frames_ids[clip_id : clip_id + clip_size]
                    ...
                    pred_bbox = outputs["pred_boxes"][0]    # [t, q, 4]
                    ...
                    pred_bbox = pred_bbox[range(clip_len), max_inds]
                ...
                all_pred_bboxes = torch.cat(all_pred_bboxes, dim=0) # (video_len, 4)
                anno_logits.append(all_pred_logits) 
                anno_masks.append(all_pred_masks)
                anno_bboxes.append(all_pred_bboxes)   
            ...
            anno_bboxes = torch.stack(anno_bboxes)   # [num_obj, video_len, 4]
            ...
            for t, frame in enumerate(frames):
                ...
                for obj in range(num_obj):
                    draw_boxes.append(anno_bboxes[obj][t].unsqueeze(0))
                draw_boxes = torch.cat(draw_boxes, dim=0)
                
                draw_boxes = rescale_bboxes(draw_boxes.detach(), (origin_w, origin_h)).tolist()

                # draw boxes
                for idx in range(num_obj):
                    xmin, ymin, xmax, ymax = draw_boxes[idx]
                    draw.rectangle(((xmin, ymin), (xmax, ymax)), outline=tuple(color_list[idx%len(color_list)]), width=2)
                
                # save_visualize_path_prefix = "/home/xhu/Code/ReferFormer/davis_dirs"
            ...
            # out_bboxes = anno_bboxes.detach().cpu().numpy().astype(np.uint8)
            # print(out_bboxes)
            # save results
